...
% VAMOS, QUE DE AQUÍ NO SACAMOS NINGUNA GRÁFICA PORQUE SE VA TODO A LA
% MIERDA SI SOLO HACES TRACKING DE LA ALTURA. VAMOS A ALGÚN OTRO FICHERO
% PARA HACER TRACK DE AMBAS. NOS VAMOS AL MODELO NO LINEAL PARA HACER
% ESO, CLARO, QUE YA NO CONSIDERAMOS SOLO LA ONDA CUADRADA QUE SÍ ESTÁ
% CERCA EN VELOCIDAD.
...
x0t(5) = h0t;
StopTime = 50;
out = sim('UAVLinCL_tracking_Hramp.slx');
...
elseif caso==3
    ...
    myxlabel('t')
    myylabel('h')
    legend({'H', '$r(t)$'}, Interpreter="latex", FontSize=12)
    myplotformat
    % mysave('cap3_trackV_c.pdf')
end
...
%%
figure(4)
plot(out.tout, out.outU(:,:), linewidth=1)
% xlim([0,35])
nfig = 2;
myxlabel('t')
myylabel('uabs')
